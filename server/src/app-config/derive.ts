...
export function deriveApp(raw: RawEnv) {
  ...
  return {
    ...
    /** Resolved: lowercased, validated against the supported set, falls back to 'zh' (src/config.ts semantics). */
    defaultLanguage: resolveDefaultLanguage(raw.DEFAULT_LANGUAGE),
  };
}

function resolveDefaultLanguage(raw: string | undefined): string {
  const lang = raw?.toLowerCase() || 'zh';
  ...
  // fell back to the default for a supported language — deliberate fix.
