...
- **Place search (standalone PWA)**: Supabase `place-search` tries Baidu (`BAIDU_MAPS_AK`), then Google Places (`GOOGLE_PLACES_API_KEY`), then OpenStreetMap. Set the keys as Supabase Edge Function secrets; missing keys skip that provider. For a Baidu server AK configured with SN validation, also set its matching Security Key as `BAIDU_MAPS_SK`; otherwise Baidu returns status 211 and search falls back. Search returns `name`, `address`, WGS84 `latitude`/`longitude`, `source`, and `externalPlaceId`. Provider request counts and last errors are logged per warm Edge isolate; no quota database is required.
