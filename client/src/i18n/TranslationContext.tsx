...
// Only the active locale is fetched; the product default (zh) and the per-key
// fallback (en) are available synchronously so first paint does not flash English.
...
export function TranslationProvider({ children }: TranslationProviderProps) {
  const language = useSettingsStore((s) => s.settings.language) || 'zh'
  const [strings, setStrings] = useState<TranslationStrings>(() => language === 'zh' ? zh : en)
