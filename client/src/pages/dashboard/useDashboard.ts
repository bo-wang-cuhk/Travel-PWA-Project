...
export function useDashboard() {
  ...
  const retryLoad = () => {
    if (!STANDALONE_MODE) loadUser({ silent: true })
    loadTrips()
  }
  ...
  const handleUpdate = async (tripData: TripCreateRequest) => {
    ...
    try {
      const data = await tripRepo.update(editingTrip.id, tripData)
  ...
  const handleArchive = async (id: number) => {
    try {
      const data = await tripRepo.archive(id)
  ...
  const handleUnarchive = async (id: number) => {
    try {
      const data = await tripRepo.unarchive(id)
  ...
  const confirmCopy = async () => {
    if (!copyTrip) return
    try {
      const title = `${copyTrip.title} (${t('dashboard.copySuffix')})`
      const data = STANDALONE_MODE
        ? await tripRepo.create({
            ...
            currency: copyTrip.currency,
            reminder_days: copyTrip.reminder_days,
          }, currentUser ? { id: currentUser.id, username: currentUser.username } : undefined)
        : await tripsApi.copy(copyTrip.id, { title })
      setTrips(prev => sortTrips([data.trip, ...prev]))
