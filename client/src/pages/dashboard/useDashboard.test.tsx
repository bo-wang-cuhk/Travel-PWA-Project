...
describe('useDashboard', () => {
  ...
  it('FE-HOOK-DASH-004: a failing trip load raises the error banner and toasts', async () => {
    vi.spyOn(tripRepo, 'list').mockRejectedValueOnce(new Error('boom'));
    const { result } = await mountLoaded();
  ...
  it('FE-HOOK-DASH-007: creating a trip prepends it and returns the payload', async () => {
    ...
    expect(payload).toMatchObject({ trip: { title: 'Iceland', id: expect.any(Number) } });
    expect((await offlineDb.trips.toArray()).some(t => t.title === 'Iceland')).toBe(true);
  ...
  it('FE-HOOK-DASH-008: a rejected create surfaces the API message', async () => {
    vi.spyOn(tripRepo, 'create').mockRejectedValueOnce(new Error('Title taken'));
    const { result } = await mountLoaded();
  ...
  it('FE-HOOK-DASH-011: a rejected update surfaces the API message', async () => {
    vi.spyOn(tripRepo, 'update').mockRejectedValueOnce(new Error('Locked'));
    const { result } = await mountLoaded();
  ...
  it('FE-HOOK-DASH-014: a failing delete toasts and keeps the trip', async () => {
    vi.spyOn(tripRepo, 'delete').mockRejectedValueOnce(new Error('nope'));
    const { result } = await mountLoaded();
  ...
  it('FE-HOOK-DASH-016: a failing archive only toasts', async () => {
    vi.spyOn(tripRepo, 'archive').mockRejectedValueOnce(new Error('nope'));
    const { result } = await mountLoaded();
  ...
  it('FE-HOOK-DASH-018: a failing restore only toasts', async () => {
    vi.spyOn(tripRepo, 'unarchive').mockRejectedValueOnce(new Error('nope'));
    const { result } = await mountLoaded();
