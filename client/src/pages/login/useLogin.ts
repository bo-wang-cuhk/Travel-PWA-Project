...
export function useLogin() {
  ...
  // Language selection chain (runs once on mount, only if user has no saved preference):
  // 1. localStorage → already in store initial state, skip
  // 2. Server default (DEFAULT_LANGUAGE env var)
  // 3. 'zh' → built-in fallback already in store
  // Browser/OS detection is deliberately not applied: a fresh installation has
  // a stable Simplified Chinese default, while an explicit choice remains sticky.
  useEffect(() => {
