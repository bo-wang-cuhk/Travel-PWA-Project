...
beforeEach(() => {
  resetAllStores();
  // Most hook cases assert English copy explicitly; language selection itself
  // is covered in the dedicated block below.
...
describe('useLogin — language detection', () => {
  it('FE-LOGIN-HOOK-022: uses the server default instead of the browser language', async () => {
    setLanguages(['de-DE', 'de']);
    server.use(http.get('/api/config', () => HttpResponse.json({ defaultLanguage: 'zh' })));
    const { result } = renderLogin();
    await ready(result);
  ...
  it('FE-LOGIN-HOOK-023: adopts a configured server default', async () => {
    setLanguages(['xh-ZA']);
  ...
  it('FE-LOGIN-HOOK-024: leaves the language alone when the server has no default either', async () => {
    ...
    useSettingsStore.setState((state) => ({ settings: { ...state.settings, language: 'zh' } }));
