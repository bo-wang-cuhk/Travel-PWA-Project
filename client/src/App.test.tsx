...
describe('RootRedirect — startup destination', () => {
  async function seedLocalActiveTrip() {
    const today = new Date().toISOString().slice(0, 10)
    await offlineDb.trips.put(buildTrip({ id: 42, title: 'Japan', start_date: today, end_date: today }))
  }
  ...
  it('FE-COMP-APP-028: falls back to the dashboard when the lookup fails', async () => {
    ...
    vi.spyOn(tripRepo, 'active').mockRejectedValueOnce(new Error('IndexedDB unavailable'))
