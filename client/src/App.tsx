...
function RootRedirect() {
  ...
  const activeLookupStarted = useRef(false)
  ...
  useEffect(() => {
    if (isLoading || !isAuthenticated || target || activeLookupStarted.current) return
...
export default function App() {
  ...
  useEffect(() => {
    if (!STANDALONE_MODE && isAuthenticated) {
      loadSettings()
      loadAddons()
      loadPlugins()
    }
